...
A converse of the last proposition exists. The proof follows directly from Lemma~19.18. 
...
\begin{proof}
The proof is similar to the proof of Theorem~19.19.  We will use mathematical induction on the degree of $f(x)$.  If the degree of $f(x)$ is 0 or 1, then $E = F$ and there is nothing to show.  Assume
...
over a subfield of order $p$.  Therefore, $E$ must also be the splitting field of $x^{p^m} - x$ over $F$.  Applying Theorem~21.5, we find that $|G(E/F)| = k$.     
...
$$
by Lemma 20.3.  Also, it is easy to show that $\sigma(\alpha \beta) = \sigma( \alpha ) \sigma( \beta )$. Since $\sigma$ is a nonzero homomorphism of fields, it must be injective.  It must also be onto, since $E$ is a finite field.  We know that $\sigma$ must be in $G(E/F)$, since $F$ is the splitting field of $x^{p^n} - x$ over the base field of order $p$. This means that $\sigma$ leaves every element in $F$ fixed.  Finally, we must show that the order of $\sigma$ is $k$. By Theorem~21.5, we know that $\sigma^k( \alpha ) = \alpha^{p^k} = \alpha$ is the identity of $G( E/F)$.  However, $\sigma^r$ cannot be the identity for $1 \leq r < k$; otherwise, $x^{p^{rk}} - x$ would have $p^m$ roots, which is impossible. 
\end{proof}
...
\sqrt{5}\, )$ over ${\mathbb Q}$ in Example~2 is indeed isomorphic to
...
f(x) = x^4 + x^3 + x^2 + x + 1
$$
over ${\mathbb Q}$. We know that $f(x)$ is irreducible by Exercise~19 in
Chapter~15.  Furthermore, since $(x -1)f(x) = x^5 -1$, we can use
...
\begin{proposition}
Let $f(x)$ be an irreducible polynomial over $F[x]$. If the
...
$$
Corollary~20.9 tells us that there exists a primitive element for any 
...
Proposition 21.11, $E_{G(E/F)} = F$. Since $| G(E/F)| = [E:F]$, this is
a finite group. 
...
By Proposition 21.3, automorphisms in $G$ permute the roots of $f(x)$
...
In Example 2 we examined the automorphisms of ${\mathbb Q}( \sqrt{3},
...
\put(7,1){\line(1,1){2}}
\put(5,1){\line(-1,1){2}}
\put(6,5.2){\line(0,1){2}}
\put(7,7.2){\line(1,-1){2}}
\put(5,7.2){\line(-1,-1){2}}
\put(17,8){${\mathbb Q}(\sqrt{3}, \sqrt{5}\, )$}
\put(14,4){${\mathbb Q}(\sqrt{3}\, )$}
\put(17.5,4){${\mathbb Q}(\sqrt{5}\, )$}
\put(21,4){${\mathbb Q}(\sqrt{15}\, )$}
\put(18.7,0){${\mathbb Q}$}
\put(19,1){\line(0,1){2}}
\put(20,1){\line(1,1){2}}
\put(18,1){\line(-1,1){2}}
\put(19,5.2){\line(0,1){2}}
\put(20,7.2){\line(1,-1){2}}
\put(18,7.2){\line(-1,-1){2}}
\end{picture}
\end{center}
...
(2)
By Proposition 21.5, $|G(E/K)| = [E:K]$; therefore, 
$$
...
\put(5.7,0.3){\vector(1,0){2.2}}
\put(5.7,3.3){\vector(1,0){2.2}}
\put(5.7,6.3){\vector(1,0){2.2}}
\put(5.7,9.3){\vector(1,0){2.2}}
\end{picture}
\end{center}
...
\setlength{\unitlength}{.18in}
\begin{picture}(25,10)
\put(12.2,0){\small ${\mathbb Q}$}
\put(11.6,3){\small ${\mathbb Q}(i)$}
\put(6.5,3){\small ${\mathbb Q}(\sqrt{2}\, )$}
\put(16.4,3){\small ${\mathbb Q}( \sqrt{2}\, i)$}
\put(11,6){\small ${\mathbb Q}(\sqrt{2}, i)$}
\put(2,6){\small ${\mathbb Q}(\sqrt[4]{2}\, )$}
\put(6,6){\small ${\mathbb Q}(\sqrt[4]{2}\, i)$}
\put(15,6){\small ${\mathbb Q}((1+i)\sqrt[4]{2}\, )$}
\put(20,6){\small ${\mathbb Q}((1-i)\sqrt[4]{2}\, )$}
\put(11.5,9){\small ${\mathbb Q}(\sqrt[4]{2}\, )$}
\put(12.5,1){\line(0,1){1.5}}
\put(11,1){\line(-2,1){2.5}}
\put(14,1){\line(2,1){2.5}}
\put(12.5,4){\line(0,1){1.5}}
\put(7.5,4){\line(0,1){1.5}}
\put(8.5,4){\line(2,1){2.5}}
\put(6.5,4){\line(-2,1){2.5}}
\put(17.5,4){\line(0,1){1.5}}
\put(18.5,4){\line(2,1){2.5}}
\put(16.5,4){\line(-2,1){2.5}}
\put(12.5,7){\line(0,1){1.5}}
\put(11,8.5){\line(-2,-1){2.5}}
\put(14,8.5){\line(2,-1){2.5}}
\put(8.5,8.5){\line(-3,-1){4.5}}
\put(16.5,8.5){\line(3,-1){4.5}}
\put(20,0){(a)}
\end{picture}
\begin{picture}(25,12)
\put(12,0){\small $\{ id \}$}
\put(11.3,3){\small $\{ id, \sigma^2 \}$}
\put(16.25,3){\small $\{ id, \sigma \tau \}$}
\put(5.9,3){\small $\{ id, \sigma^2 \tau  \}$}
\put(10.4,6){\small $\{ id, \sigma, \sigma^2, \sigma^3 \}$}
\put(2.5,3){\small $\{ id, \tau \}$}
\put(4,6){\small $\{ id, \sigma^2, \tau, \sigma^2 \tau \}$}
\put(16,6){\small $\{ id, \sigma^2, \sigma \tau, \sigma^3 \tau \}$}
\put(20,3){\small $\{id, \sigma^3 \tau  \}$}
\put(12,9){\small $D_4$}
\put(12.5,1){\line(0,1){1.5}}
\put(11,1){\line(-2,1){2.5}}
\put(14,1){\line(2,1){2.5}}
\put(12.5,4){\line(0,1){1.5}}
\put(7.5,4){\line(0,1){1.5}}
\put(8.5,5.5){\line(2,-1){2.5}}
\put(6.5,5.5){\line(-2,-1){2.5}}
\put(17.5,4){\line(0,1){1.5}}
\put(18.5,5.5){\line(2,-1){2.5}}
\put(16.5,5.5){\line(-2,-1){2.5}}
\put(12.5,7){\line(0,1){1.5}}
\put(11,8.5){\line(-2,-1){2.5}}
\put(14,8.5){\line(2,-1){2.5}}
\put(8.5,1){\line(-3,1){4.5}}
\put(16.5,1){\line(3,1){4.5}}
\put(20,0){(b)}
\end{picture}
\end{center}
...
a$. As we discussed in Chapter~3, the roots of $x^n - 1$ are called
the {\bfi nth roots of unity}\index{$n$th root of unity}.  These
roots are a finite subgroup of the splitting field of $x^n -1$. By
Theorem~20.7, the $n$th roots of unity form a cyclic group.  Any
...
solvable.  On the other hand, $S_5$ is not solvable, by Theorem~9.8.
...
know by Lemma~21.17 that $E$ is a normal extension of each $F_i$. By
...
$$
By Lemma~21.16, $G(F_i/F_{i-1})$ is solvable; hence, $G(E/F)$ is also
solvable. 
\mbox{\hspace{1in}}
\end{proof}
...
\begin{center}
\centerline {
\includegraphics[width=2in]{Galois4}
}
\end{center}
...
S_5$. By Lemma~21.19, $S_5$ is generated by a transposition and an
element of order 5; therefore, $G(K/F)$ must be all of $S_5$. By
Theorem~9.8, $S_5$ is not solvable. Consequently, $f(x)$ cannot be
solved by radicals.  
\hspace{\fill} $\blacksquare$
...
\item[{\bf [1]}]
Artin, E. {\it Galois Theory}. 2nd ed. University of
Notre Dame, Notre Dame, IN, 1944.
...
\item[{\bf [3]}]
Fraleigh, J. B. {\it A First Course in Abstract Algebra},
4th ed. Addison-Wesley, Reading, MA, 1989.
 
\item[{\bf [4]}]
Gaal, L. {\it Classical Galois Theory with Examples}. 2nd
ed. Chelsea, New York, 1973.
