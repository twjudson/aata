...
%%%%(c)
%%%%(c)
\chapter{Cosets and Lagrange's Theorem}{cosets}
...
\item
Modify the proof of Theorem~\ref{cosets_theorem_9} to show that any two permutations $\alpha, \beta \in S_n$ have the same cycle structure if and only if there exists a  permutation $\gamma$ such that $\beta = \gamma \alpha \gamma^{-1}$.  If $\beta = \gamma \alpha \gamma^{-1}$ for some $\gamma \in S_n$, then $\alpha$ and $\beta$ are {\bfi conjugate}\index{Conjugate permutations}\index{Permutation!conjugate}.
